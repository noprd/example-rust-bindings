...
def unique_id(
    *,
    ...
    NOTE: uses `uuid7` as the contains a recoverable encoding of the timestamp + random bits.
    """
    code = uuid7().hex
    code = (prefix or "") + code + (suffix or "")
    if random_suffix:
        bits = str(round(1e4 * (1 + random())))
        bits = bits[1:]
        code = code + f"r{bits}"
    return code
...
def first_non_empty(
    *values: T | None,
    default: T,
    empty: list[T | None] | None = None,
) -> T:
    """
    Computes the first non-empty value
    """
    empty = empty or []
    value = next(
        (
...
def flatten(
    ...
    *,
    ...
    Optionally ensures uniqueness of elements
    """
    X_flat = []
    if unique:
        for XX in X:
