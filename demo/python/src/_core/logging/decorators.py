    ...
    *,
    ...
    def dec(
        ...
        def wrapped_action(*_: PARAMS.args, **__: PARAMS.kwargs) -> RETURN:
            ...
            message_end, message_error = echo_beginning(_, __, timer=timer, isolated=isolated, depth=depth, close=close, message=message_, level=level)  # fmt: skip

            try:
                output = action(*_, **__)
                match output:
                    case Err():
                        echo_end(timer=timer, isolated=isolated, close=close, message=message_error, level=level)  # fmt: skip
                        return output

                    # case Ok() | _:
                    case _:
                        echo_end(timer=timer, isolated=isolated, close=close, message=message_end, level=level)  # fmt: skip
...
def echo_beginning(
    ...
    *,
    ...
    if isolated:
        ...
        # either pick latest value or forced value
        depth = depth or depths[-1]
